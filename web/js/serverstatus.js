	...
	$.getJSON("json/stats.json", function(result) {
		...
		for (var i = 0; i < result.servers.length; i++) {
			...
			if (!result.servers[i].online4 && !result.servers[i].online6) {
				...
				ExpandRow[0].children["expand_mem"].innerHTML = "内存信息: " + bytesToSize(result.servers[i].memory_used, 2) + " / " + bytesToSize(result.servers[i].memory_total, 2);
				// Swap
				ExpandRow[0].children["expand_swap"].innerHTML = "交换分区: " + bytesToSize(result.servers[i].swap_used, 2) + " / " + bytesToSize(result.servers[i].swap_total, 2);
				...
				ExpandRow[0].children["expand_hdd"].innerHTML = "硬盘信息: " + bytesToSize(result.servers[i].hdd_used, 2) + " / " + bytesToSize(result.servers[i].hdd_total, 2);
